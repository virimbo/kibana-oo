...
def patched_es(monkeypatch):
    ...
    async def fake_es(sid, index, body):
        ...
        if qs:  # NVS pipeline docs query — 4 hits, 3 unique documents (n1 appears twice)
            return {"hits": {"hits": [
                {"_source": {"@timestamp": "t1", "url": {"path": "/document/n1"}, "title": "N1"}},
                {"_source": {"@timestamp": "t2", "url": {"path": "/document/n1"}, "title": "N1"}},
                {"_source": {"@timestamp": "t3", "url": {"path": "/document/n2"}, "title": "N2"}},
                {"_source": {"@timestamp": "t4", "url": {"path": "/document/n3"}, "title": "N3"}}]}}
...
async def test_build_snapshot_assembles_consistent_payload(patched_es):
    ...
    assert snap.nvs_count == 3    # unique NVS documents (deduplicated from 4 hits)
    assert len(snap.nvs_docs) == snap.nvs_count   # tile number always matches the list
    assert snap.nvs_docs[0]["link"].endswith("/document/n1")
